...
/**
 * Handle achievement role user selection
 */
export async function handleAchievementRoleUserSelect(
  ...
  try {
    ...
    ConfigManager.updateGuildConfig(guildId, {
      ...
      points: {
        ...pointsConfig,
        achievementRoleUser: selectedRole.id,
      },
    });
    ...
    if (channel && channel.isTextBased()) {
      ...
      await interaction.reply({
        content: `✅ Achievement role user set to <@&${selectedRole.id}>!`,
        flags: MessageFlags.Ephemeral,
      });
      return;
    }
  } catch (error) {
    console.error("Error setting achievement role user:", error);
    await interaction.reply({
      content: "❌ Failed to set achievement role user. Please try again.",
      flags: MessageFlags.Ephemeral,
    });
  }
}
